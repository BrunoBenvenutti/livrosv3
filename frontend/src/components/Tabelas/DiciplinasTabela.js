...
export default function Tabela(props) {
  ...
  function getLinhas() {
    ...
    return arrayRegistros.map((item, i) => {
      ...
      return (
        <tr className={i % 2 === 0 ? "par" : "impar"} key={item.dis_codigo}>
          <td> {item.dis_codigo} </td>
          <td> {item.dis_nome} </td>
          <td> {item.dis_apelido} </td>
          <td> {status} </td>
          <td> {item.dis_sexo} </td>
          <td> {item.dis_telefone} </td>
          <td> {item.dis_email} </td>

          <td id="editar"> <a className="btn btn-primary btn-block" href={props.chave + item.dis_codigo} > Editar </a></td>
          <td id="ativar"> <a className="btn btn-danger btn-block" href={props.chave + item.dis_codigo} > Inativar </a></td>
      ...
      <table id="tabela" className="table table-hover">
        <thead id="cabecalho_rel">
          <tr style={{ textAlign: 'left' }}>
            ...
            <th scope="col"><a href={props.chave + '0'} className="btn btn-success btn-block">Nova Diciplina</a></th>
          </tr>
        </thead>
        <tbody>
          {getLinhas()}
